  ...
  try {
    ...
    if (!userId) {
      return res.status(401).json({
        message: "User not authenticated or reference website missing.",
      });
    }
        ...
        items: [
          {
            ...
            total: product.actualPrice * quantity,
          },
        ],
...
export const updateCartItemQuantity = async (req, res) => {
  try {
    ...
    if (
      !req.user?.id ||
      !req.user?.referenceWebsite ||
      !productId ||
      quantity === undefined
    ) {
      return res
        .status(400)
        .json({ message: "User, Product ID, and Quantity are required." });
    }
    ...
    if (isNaN(parsedQuantity) || parsedQuantity <= 0) {
      return res
        .status(400)
        .json({ message: "Quantity must be a valid positive number." });
    }
    ...
    if (!cart) {
      return res
        .status(404)
        .json({ message: "Cart not found for the user and website." });
    }
    ...
    if (!item.actualPrice === undefined) {
      return res.status(400).json({
        message: "Product actualPrice is missing from the cart item.",
      });
    }
    ...
    if (isNaN(actualPrice) || actualPrice < 0) {
      return res
        .status(400)
        .json({ message: `Invalid actual price: ${item.actualPrice}` });
    }
    ...
    if (isNaN(totalPrice)) {
      return res.status(500).json({
        message: `Failed to calculate total price: ${parsedQuantity} * ${actualPrice} = NaN`,
      });
    }
...
export const removeItemFromCart = async (req, res) => {
  try {
    ...
    if (!identifier || !productId) {
      return res
        .status(400)
        .json({ message: "Identifier and Product ID are required." });
    }
...
export const getCart = async (req, res) => {
  try {
    ...
    const cart = await Cart.findOne({ identifier }).populate(
      "items.product",
      "productName images price actualPrice discount"
    );
    if (!cart) {
      return res
        .status(404)
        .json({ message: "No cart found for the user and website." });
    }
...
export const checkoutCart = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to checkout cart", error: error.message });
  }
};
